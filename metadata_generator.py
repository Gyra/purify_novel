...
for i in ind:
    ...
    metadata = {'name': f'洁净文学 #{str(location)}',
                'description': f'符合新时代语言规范的洁净文学选段： 《{book}》第{num}回',
                'image': ipfs + f'/{str(location)}.png',
                'attributes': attr
                }
    os.rename(chapters[i], f'pic\\{str(location)}.png')
    with open(f'metadata\\{str(location)}', 'w', encoding='utf-8') as f:
